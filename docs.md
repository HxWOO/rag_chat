...
| 시나리오 ID	| 사용자 질문 예시 (T590 매뉴얼 기반) |	기대 답변 (요약) | 기대 출처 (예시) |
|------------|-------------------------------|---------------|---------------|
TC-001 (안전) | "T590 로더의 안전 장비 목록 알려줘." | "좌석 벨트, 시트 바, 운전실(ROPS/FOPS), 작동 설명서, 안전 표지판(데칼) 등이 있습니다." (Source 1.1, 1.2)	| "매뉴얼 [SAFETY EQUIPMENT] 섹션" |
TC-002 (유지보수) |	"엔진 오일 레벨은 언제 점검해야 해?" |	"매 10시간 작동 전 또는 매일 점검해야 합니다." (Source 2.1) |	"매뉴얼 [REGULAR MAINTENANCE ITEMS] 또는 [MAINTENANCE SCHEDULE] 섹션" |
TC-003 (작동법) |	"주차 브레이크는 어떻게 작동시켜?" |	"엔진을 끄거나 시트 바를 올리면 자동으로 작동(결합)됩니다. 수동으로 켜려면 스위치..." |	"매뉴얼 [OPERATING INSTRUCTIONS] 또는 [PARKING BRAKE] 섹션"
TC-004 (정보 없음) |	"이 장비 재고 언제 들어와?"	 | "관련 정보를 매뉴얼에서 찾을 수 없습니다." |	(출처 없음)
